...
/*   Updated: 2023/10/31 16:54:59 by feliciencat      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool Invite::execute(User *client, std::vector<std::string> args) {
  ...
  {
    client->response(ERR_USERONCHANNEL(client->getNickname(), args[1], args[2]));
    return false;
  }
  tmpUser->setChannelInvited(tmpChan);
  client->response(RPL_INVITING(client->getNickname(), args[1], args[2]));
  
  std::string msg_invite = "You have been invited to join channel : " + tmpChan->getChannelName() + " by " + client->getNickname();
